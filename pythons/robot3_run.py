...
def UpStairs(classNav,classPose):
  ...
  if Nav_state:
    ...
    if Nav_state:
      ...
      SetInitPose(tmp_trans,tmp_rot,"robot3")
      rospy.sleep(2)
      CtrlLift(1)
      rospy.sleep(0.5)
      CtrlLift(1)
  ...
  if Nav_state:
    ...
    if Nav_state:
      ...
      while tmp_trans == None:
        tmp_trans,tmp_rot = classPose.get_pos()
        tmp_trans[1] = tmp_trans[1]-34.65
      SetInitPose(tmp_trans,tmp_rot,"robot3") #重定位 C
      CtrlLift(0)
      rospy.sleep(0.5)
      CtrlLift(0)
      rospy.sleep(2)
...
if __name__ == '__main__':
  ...
  B_1st = {"name":"A","pose":(-5.52, -6.58, 0.0),"orientation":(0.0, 0.0, 1, -0.023),"object":2}
  A_2nd = {"name":"F","pose":(-5.32, 31.4, 0.0),"orientation":(0.0, 0.0, -0.694, -0.720),"object":0}
  rospy.init_node('robot3_display', anonymous=False)
  # Init_pose()

  navigation = GoToPose("robot3")
  robotpose = Robot("robot3")
  try:
    while True:
